...
ESCAPE_SEQUENCES_REPLACEMENTS = {'\\': '\\\\',
                                 ...
                                 '\t': '\\t',
                                 '\v': '\\v'}
METRICS = ['accuracy', 'bpc', 'loss', 'perplexity', 'op_time']
...
def get_keys_from_nested(obj, ready=None, collected=None):
    """Returns list of all paths in nested object"""
    if ready is None:
        ready = list()
...
def loop_through_file_name_indices(filename, start_index):
...
def add_index_to_filename_if_needed(filename, index=None):
    if index is not None:
        return loop_through_file_name_indices(filename, index)
    if os.path.exists(filename):
        return loop_through_file_name_indices(filename, 1)
    return filename
...
def unite_ordered_dicts(list_of_dicts):
...
def unite_nested_dicts(list_of_nested, depth):
    if isinstance(list_of_nested[0], dict):
        ...
        raise InvalidArgumentError(
            "list_of_nested has to be a list of dicts or a list of OrderedDicts",
            list_of_nested,
            'list_of_nested',
...
def sort_lists_map(lists):
    ...
    new_values = list()
    for v in values:
        str_and_ints_are_present = not (cum_and([isinstance(elem, str) for elem in v]) or
                                        cum_and([isinstance(elem, int) for elem in v]))
        if str_and_ints_are_present:
...
def nth_elements_of_sequences_in_sequence_of_sequences(s, n):
    res = list()
    for el in s:
        res.append(el[n])
    return res
...
def check_header_line_is_present(save_path):
    try:
        with open(save_path, 'r') as f:
            top_line = f.read().split('\n')[0]
        if check_if_line_contains_results(top_line):
            return False
...
def write_line(metrics, params, save_path, clean=False):
    if clean:
        mode = 'w'
    else:
        mode = 'a'
